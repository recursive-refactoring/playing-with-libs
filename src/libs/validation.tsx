...
// ---------------- ARRAY ----------------
export function createArrayBuilder() {
  ...
  const api = {
    ...
    required(message?: string) {
      schema = schema.required(
        message ?? `${labelText ?? "Field"} is required`,
      );
      return api;
    },
    ...
    when(
      keys: string | string[],
      options: Parameters<Yup.ArraySchema<any>["when"]>[1],
    ) {
...
// ---------------- OBJECT ----------------
export function createObjectBuilder() {
  ...
  const api = {
    ...
    noUnknown(message?: string) {
      schema = schema.noUnknown(message);
      return api;
    },
...
export function schemaBuilder() {
  ...
  const api = {
    ...
    required(message: string = `${labelText ?? "This field"} is required`) {
      schema = schema.required(message);
      return api;
    },

    email(message: string = `${labelText ?? "This field"} is invalid email`) {
      schema = schema.email(message);
      return api;
    },

    url(message: string = `${labelText ?? "This field"} is invalid url`) {
